...
public sealed partial class RequestHandlerSourceGenerator
{
    private static void GenerateMediaThorDispatcher(SourceProductionContext context,
        ImmutableArray<RequestHandlerInformation> genericRequestHandlersInformation, ImmutableArray<RequestHandlerInformation> streamRequestHandlerInformation)
    {
        ...
        using (writer.CreateScope())
        {
            ...
            writer
                .AppendLine()
                .AppendLine(
                    "public Task DispatchAsync<TRequest>(IServiceProvider serviceProvider, TRequest request, CancellationToken cancellationToken)")
                .AppendIndentedLine("where TRequest : IRequest");
            using (writer.CreateScope())
            {
                writer
                    .AppendLine("RequestHandlerDelegate pipeline = token => serviceProvider.GetRequiredService<IRequestHandler<TRequest>>().HandleAsync(request, token);")
                    .AppendLine()
                    .AppendLine("foreach (var behavior in serviceProvider.GetServices<IPipelineBehavior<TRequest>>().Reverse())");
                using (writer.CreateScope())
                {
                    writer
                        .AppendLine("var next = pipeline;")
                ...
                writer
                    .AppendLine()
                    .AppendLine("return pipeline.Invoke(cancellationToken);");
            }
            
            writer
                .AppendLine()
                .AppendLine(
                "public Task<TResponse> DispatchAsync<TResponse>(IServiceProvider serviceProvider, IRequest<TResponse> request, CancellationToken cancellationToken)");
            using (writer.CreateScope())
            {
                ...
                foreach (var handledType in genericRequestHandlersInformation.SelectMany(r => r.HandledTypes))
                {
            ...
            writer
                .AppendLine()
                .AppendLine(
                    "public async IAsyncEnumerable<TResponse> DispatchAsync<TResponse>(IServiceProvider serviceProvider, IStreamRequest<TResponse> request, [EnumeratorCancellation] CancellationToken cancellationToken)");
            using (writer.CreateScope())
            {
        ...
        ImmutableArray<RequestHandlerInformation> requestHandlersInformation)
    {
        ...
        using (writer.CreateScope())
        {
            ...
            {
                writer
                    .AppendLine("services.AddTransient<MediaThor.IMediator, MediaThor.MediaThor>();")
